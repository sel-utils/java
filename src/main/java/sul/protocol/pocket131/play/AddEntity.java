...
public class AddEntity extends Packet {
	...
	public float pitch;
	public float yaw;
	public sul.protocol.pocket131.types.Attribute[] attributes = new sul.protocol.pocket131.types.Attribute[0];
	public sul.metadata.Pocket131 metadata;
	public sul.protocol.pocket131.types.Link[] links = new sul.protocol.pocket131.types.Link[0];

	public AddEntity() {}

	public AddEntity(long entityId, long runtimeId, int type, Tuples.FloatXYZ position, Tuples.FloatXYZ motion, float pitch, float yaw, sul.protocol.pocket131.types.Attribute[] attributes, sul.metadata.Pocket131 metadata, sul.protocol.pocket131.types.Link[] links) {
	...
	@Override
	public int length() {
		int length=Buffer.varlongLength(entityId) + Buffer.varlongLength(runtimeId) + Buffer.varuintLength(type) + Buffer.varuintLength(attributes.length) + metadata.length() + Buffer.varuintLength(links.length) + 33; for(sul.protocol.pocket131.types.Attribute yrcldrc:attributes){ length+=yrcldrc.length(); };for(sul.protocol.pocket131.types.Link blam:links){ length+=blam.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)links.length); for(sul.protocol.pocket131.types.Link blam:links){ this.writeBytes(blam.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bxbt=this.readVaruint(); links=new sul.protocol.pocket131.types.Link[bxbt]; for(int blam=0;blam<links.length;blam++){ links[blam]=new sul.protocol.pocket131.types.Link(); links[blam]._index=this._index; links[blam].decode(this._buffer); this._index=links[blam]._index; }
	}
