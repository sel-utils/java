...
public class InventoryContent extends Packet {
	...
	public int window;
	public sul.protocol.pocket131.types.Slot[] slots = new sul.protocol.pocket131.types.Slot[0];

	public InventoryContent() {}

	public InventoryContent(int window, sul.protocol.pocket131.types.Slot[] slots) {
		this.window = window;
		this.slots = slots;
	}
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(window) + Buffer.varuintLength(slots.length) + 1; for(sul.protocol.pocket131.types.Slot cxdm:slots){ length+=cxdm.length(); } return length;
	}
