...
public class AvailableCommands extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(enumValues.length) + Buffer.varuintLength(unknown1) + Buffer.varuintLength(commands.length) + 2; for(String z5bzbvc:enumValues){ length+=Buffer.varuintLength(z5bzbvc.getBytes(StandardCharsets.UTF_8).length)+z5bzbvc.getBytes(StandardCharsets.UTF_8).length; };for(sul.protocol.pocket131.types.Command y9bfzm:commands){ length+=y9bfzm.length(); } return length;
	}
