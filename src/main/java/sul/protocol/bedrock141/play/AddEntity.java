...
public class AddEntity extends Packet {
	...
	public AddEntity(long entityId, long runtimeId, int type, Tuples.FloatXYZ position, Tuples.FloatXYZ motion, float pitch, float yaw, sul.protocol.bedrock141.types.Attribute[] attributes, sul.metadata.Bedrock141 metadata, sul.protocol.bedrock141.types.Link[] links) {
	...
	@Override
	public int length() {
		int length=Buffer.varlongLength(entityId) + Buffer.varulongLength(runtimeId) + Buffer.varuintLength(type) + Buffer.varuintLength(attributes.length) + metadata.length() + Buffer.varuintLength(links.length) + 33; for(sul.protocol.bedrock141.types.Attribute yrcldrc:attributes){ length+=yrcldrc.length(); };for(sul.protocol.bedrock141.types.Link blam:links){ length+=blam.length(); } return length;
	}
