...
public class NodeInfo extends Packet {
	 ...
	 */
	public sul.protocol.hncom2.types.Plugin[] plugins = new sul.protocol.hncom2.types.Plugin[0];
	...
	public NodeInfo(long time, int max, sul.protocol.hncom2.types.Game[] acceptedGames, sul.protocol.hncom2.types.Plugin[] plugins, String additionalJson) {
		this.time = time;
		this.max = max;
	...
	@Override
	public int length() {
		int length=Buffer.varulongLength(time) + Buffer.varuintLength(max) + Buffer.varuintLength(acceptedGames.length) + Buffer.varuintLength(plugins.length) + Buffer.varuintLength(additionalJson.getBytes(StandardCharsets.UTF_8).length) + additionalJson.getBytes(StandardCharsets.UTF_8).length + 1; for(sul.protocol.hncom2.types.Game ynzbzry1:acceptedGames){ length+=ynzbzry1.length(); };for(sul.protocol.hncom2.types.Plugin cxzlc:plugins){ length+=cxzlc.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)plugins.length); for(sul.protocol.hncom2.types.Plugin cxzlc:plugins){ this.writeBytes(cxzlc.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bbddbm=this.readVaruint(); plugins=new sul.protocol.hncom2.types.Plugin[bbddbm]; for(int cxzlc=0;cxzlc<plugins.length;cxzlc++){ plugins[cxzlc]=new sul.protocol.hncom2.types.Plugin(); plugins[cxzlc]._index=this._index; plugins[cxzlc].decode(this._buffer); this._index=plugins[cxzlc]._index; }
