...
public class InventoryTransaction extends Packet {
	...
	public int type;
	public sul.protocol.bedrock160.types.InventoryAction[] actions = new sul.protocol.bedrock160.types.InventoryAction[0];

	public InventoryTransaction() {}

	public InventoryTransaction(int type, sul.protocol.bedrock160.types.InventoryAction[] actions) {
		this.type = type;
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(type) + Buffer.varuintLength(actions.length) + 1; for(sul.protocol.bedrock160.types.InventoryAction yna9c:actions){ length+=yna9c.length(); } return length;
	}
	...
	private byte[] encodeImpl() {
		...
		this.writeVaruint((int)actions.length); for(sul.protocol.bedrock160.types.InventoryAction yna9c:actions){ this.writeBytes(yna9c.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bfdlbm=this.readVaruint(); actions=new sul.protocol.bedrock160.types.InventoryAction[bfdlbm]; for(int yna9c=0;yna9c<actions.length;yna9c++){ actions[yna9c]=new sul.protocol.bedrock160.types.InventoryAction(); actions[yna9c]._index=this._index; actions[yna9c].decode(this._buffer); this._index=actions[yna9c]._index; }
	}
	...
	public class UseItem extends Packet {
		...
		public UseItem(int actionType, sul.protocol.bedrock160.types.BlockPosition blockPosition, int face, int hotbarSlot, sul.protocol.bedrock160.types.Slot item, Tuples.FloatXYZ playerPosition, Tuples.FloatXYZ clickPosition) {
		...
		public void decode(byte[] buffer) {
			...
			blockPosition=new sul.protocol.bedrock160.types.BlockPosition(); blockPosition._index=this._index; blockPosition.decode(this._buffer); this._index=blockPosition._index;
			face=this.readVarint();
			hotbarSlot=this.readVarint();
			item=new sul.protocol.bedrock160.types.Slot(); item._index=this._index; item.decode(this._buffer); this._index=item._index;
	...
	public class UseItemOnEntity extends Packet {
		...
		public UseItemOnEntity(long entityId, int actionType, int hotbarSlot, sul.protocol.bedrock160.types.Slot item, Tuples.FloatXYZ unknown4, Tuples.FloatXYZ unknown5) {
	...
	public class ReleaseItem extends Packet {
		...
		public ReleaseItem(int actionType, int hotbarSlot, sul.protocol.bedrock160.types.Slot item, Tuples.FloatXYZ headPosition) {
			this.actionType = actionType;
			this.hotbarSlot = hotbarSlot;
			this.item = item;
