...
public class ListAddPlayer extends Stream {
	...
	public UUID uuid;
	public String name;
	public sul.protocol.minecraft210.types.Property[] properties = new sul.protocol.minecraft210.types.Property[0];
	public int gamemode;
	public int latency;
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(name.getBytes(StandardCharsets.UTF_8).length) + name.getBytes(StandardCharsets.UTF_8).length + Buffer.varuintLength(properties.length) + Buffer.varuintLength(gamemode) + Buffer.varuintLength(latency) + Buffer.varuintLength(displayName.getBytes(StandardCharsets.UTF_8).length) + displayName.getBytes(StandardCharsets.UTF_8).length + 17; for(sul.protocol.minecraft210.types.Property cjcvdlc:properties){ length+=cjcvdlc.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)properties.length); for(sul.protocol.minecraft210.types.Property cjcvdlc:properties){ this.writeBytes(cjcvdlc.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bbbbcrzm=this.readVaruint(); properties=new sul.protocol.minecraft210.types.Property[bbbbcrzm]; for(int cjcvdlc=0;cjcvdlc<properties.length;cjcvdlc++){ properties[cjcvdlc]=new sul.protocol.minecraft210.types.Property(); properties[cjcvdlc]._index=this._index; properties[cjcvdlc].decode(this._buffer); this._index=properties[cjcvdlc]._index; }
