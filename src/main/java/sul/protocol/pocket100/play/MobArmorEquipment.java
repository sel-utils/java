...
public class MobArmorEquipment extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varlongLength(entityId) + 1; for(sul.protocol.pocket100.types.Slot yjbi:armor){ length+=yjbi.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		for(sul.protocol.pocket100.types.Slot yjbi:armor){ this.writeBytes(yjbi.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		final int bfb9=4; armor=new sul.protocol.pocket100.types.Slot[bfb9]; for(int yjbi=0;yjbi<armor.length;yjbi++){ armor[yjbi]=new sul.protocol.pocket100.types.Slot(); armor[yjbi]._index=this._index; armor[yjbi].decode(this._buffer); this._index=armor[yjbi]._index; }
	}
