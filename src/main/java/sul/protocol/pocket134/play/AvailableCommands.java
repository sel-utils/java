...
public class AvailableCommands extends Packet {
	...
	public sul.protocol.pocket134.types.Enum[] enums = new sul.protocol.pocket134.types.Enum[0];
	public sul.protocol.pocket134.types.Command[] commands = new sul.protocol.pocket134.types.Command[0];

	public AvailableCommands() {}

	public AvailableCommands(String[] enumValues, String[] unknown1, sul.protocol.pocket134.types.Enum[] enums, sul.protocol.pocket134.types.Command[] commands) {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(enumValues.length) + Buffer.varuintLength(unknown1.length) + Buffer.varuintLength(enums.length) + Buffer.varuintLength(commands.length) + 2; for(String z5bzbvc:enumValues){ length+=Buffer.varuintLength(z5bzbvc.getBytes(StandardCharsets.UTF_8).length)+z5bzbvc.getBytes(StandardCharsets.UTF_8).length; };for(String d5b9be:unknown1){ length+=Buffer.varuintLength(d5b9be.getBytes(StandardCharsets.UTF_8).length)+d5b9be.getBytes(StandardCharsets.UTF_8).length; };for(sul.protocol.pocket134.types.Enum z5bm:enums){ length+=z5bm.length(); };for(sul.protocol.pocket134.types.Command y9bfzm:commands){ length+=y9bfzm.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)enums.length); for(sul.protocol.pocket134.types.Enum z5bm:enums){ this.writeBytes(z5bm.encode()); }
		this.writeVaruint((int)commands.length); for(sul.protocol.pocket134.types.Command y9bfzm:commands){ this.writeBytes(y9bfzm.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bvd1=this.readVaruint(); enums=new sul.protocol.pocket134.types.Enum[bvd1]; for(int z5bm=0;z5bm<enums.length;z5bm++){ enums[z5bm]=new sul.protocol.pocket134.types.Enum(); enums[z5bm]._index=this._index; enums[z5bm].decode(this._buffer); this._index=enums[z5bm]._index; }
		int bnb1br=this.readVaruint(); commands=new sul.protocol.pocket134.types.Command[bnb1br]; for(int y9bfzm=0;y9bfzm<commands.length;y9bfzm++){ commands[y9bfzm]=new sul.protocol.pocket134.types.Command(); commands[y9bfzm]._index=this._index; commands[y9bfzm].decode(this._buffer); this._index=commands[y9bfzm]._index; }
	}
