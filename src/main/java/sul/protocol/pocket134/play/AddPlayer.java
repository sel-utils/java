...
public class AddPlayer extends Packet {
	...
	public float yaw;
	public sul.protocol.pocket134.types.Slot heldItem;
	public sul.metadata.Pocket134 metadata;
	...
	public sul.protocol.pocket134.types.Link[] links = new sul.protocol.pocket134.types.Link[0];

	public AddPlayer() {}

	public AddPlayer(sul.protocol.pocket134.types.McpeUuid uuid, String username, long entityId, long runtimeId, Tuples.FloatXYZ position, Tuples.FloatXYZ motion, float pitch, float headYaw, float yaw, sul.protocol.pocket134.types.Slot heldItem, sul.metadata.Pocket134 metadata, int unknown11, int unknown12, int unknown13, int unknown14, int unknown15, long unknown16, sul.protocol.pocket134.types.Link[] links) {
		this.uuid = uuid;
	...
	@Override
	public int length() {
		int length=uuid.length() + Buffer.varuintLength(username.getBytes(StandardCharsets.UTF_8).length) + username.getBytes(StandardCharsets.UTF_8).length + Buffer.varlongLength(entityId) + Buffer.varulongLength(runtimeId) + heldItem.length() + metadata.length() + Buffer.varuintLength(unknown11) + Buffer.varuintLength(unknown12) + Buffer.varuintLength(unknown13) + Buffer.varuintLength(unknown14) + Buffer.varuintLength(unknown15) + Buffer.varuintLength(links.length) + 45; for(sul.protocol.pocket134.types.Link blam:links){ length+=blam.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)links.length); for(sul.protocol.pocket134.types.Link blam:links){ this.writeBytes(blam.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		heldItem=new sul.protocol.pocket134.types.Slot(); heldItem._index=this._index; heldItem.decode(this._buffer); this._index=heldItem._index;
		metadata=new sul.metadata.Pocket134(); metadata._index=this._index; metadata.decode(this._buffer); this._index=metadata._index;
		...
		int bxbt=this.readVaruint(); links=new sul.protocol.pocket134.types.Link[bxbt]; for(int blam=0;blam<links.length;blam++){ links[blam]=new sul.protocol.pocket134.types.Link(); links[blam]._index=this._index; links[blam].decode(this._buffer); this._index=links[blam]._index; }
	}
