...
public class AddNode extends Packet {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(hubId) + Buffer.varuintLength(name.getBytes(StandardCharsets.UTF_8).length) + name.getBytes(StandardCharsets.UTF_8).length + Buffer.varuintLength(acceptedGames.length) + 2; for(sul.protocol.hncom1.types.Game ynzbzry1:acceptedGames){ length+=ynzbzry1.length(); } return length;
	}
