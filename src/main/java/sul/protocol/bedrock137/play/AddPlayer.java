...
public class AddPlayer extends Packet {
	...
	public AddPlayer(sul.protocol.bedrock137.types.McpeUuid uuid, String username, long entityId, long runtimeId, Tuples.FloatXYZ position, Tuples.FloatXYZ motion, float pitch, float headYaw, float yaw, sul.protocol.bedrock137.types.Slot heldItem, sul.metadata.Bedrock137 metadata, int unknown11, int unknown12, int unknown13, int unknown14, int unknown15, long unknown16, sul.protocol.bedrock137.types.Link[] links) {
		this.uuid = uuid;
	...
	@Override
	public int length() {
		int length=uuid.length() + Buffer.varuintLength(username.getBytes(StandardCharsets.UTF_8).length) + username.getBytes(StandardCharsets.UTF_8).length + Buffer.varlongLength(entityId) + Buffer.varulongLength(runtimeId) + heldItem.length() + metadata.length() + Buffer.varuintLength(unknown11) + Buffer.varuintLength(unknown12) + Buffer.varuintLength(unknown13) + Buffer.varuintLength(unknown14) + Buffer.varuintLength(unknown15) + Buffer.varuintLength(links.length) + 45; for(sul.protocol.bedrock137.types.Link blam:links){ length+=blam.length(); } return length;
	}
	...
	public void decode(byte[] buffer) {
		...
		heldItem=new sul.protocol.bedrock137.types.Slot(); heldItem._index=this._index; heldItem.decode(this._buffer); this._index=heldItem._index;
