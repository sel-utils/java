...
public class ChunkData extends Stream {
	 ...
	 */
	public sul.protocol.pocket133.types.Section[] sections = new sul.protocol.pocket133.types.Section[0];
	...
	public sul.protocol.pocket133.types.ExtraData[] extraData = new sul.protocol.pocket133.types.ExtraData[0];
	...
	public ChunkData(sul.protocol.pocket133.types.Section[] sections, short[] heights, byte[] biomes, byte[] borders, sul.protocol.pocket133.types.ExtraData[] extraData, byte[] blockEntities) {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(sections.length) + Buffer.varuintLength(borders.length) + borders.length + Buffer.varuintLength(extraData.length) + blockEntities.length + 773; for(sul.protocol.pocket133.types.Section cvdlbm:sections){ length+=cvdlbm.length(); };for(sul.protocol.pocket133.types.ExtraData zhcfyr:extraData){ length+=zhcfyr.length(); } return length;
	}

	@Override
	public byte[] encode() {
		this._buffer = new byte[this.length()];
		this.writeVaruint((int)sections.length); for(sul.protocol.pocket133.types.Section cvdlbm:sections){ this.writeBytes(cvdlbm.encode()); }
		...
		this.writeVaruint((int)extraData.length); for(sul.protocol.pocket133.types.ExtraData zhcfyr:extraData){ this.writeBytes(zhcfyr.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		this._index = 0;
		int bnyrb5=this.readVaruint(); sections=new sul.protocol.pocket133.types.Section[bnyrb5]; for(int cvdlbm=0;cvdlbm<sections.length;cvdlbm++){ sections[cvdlbm]=new sul.protocol.pocket133.types.Section(); sections[cvdlbm]._index=this._index; sections[cvdlbm].decode(this._buffer); this._index=sections[cvdlbm]._index; }
		...
		int bvdjrfy=this.readVaruint(); extraData=new sul.protocol.pocket133.types.ExtraData[bvdjrfy]; for(int zhcfyr=0;zhcfyr<extraData.length;zhcfyr++){ extraData[zhcfyr]=new sul.protocol.pocket133.types.ExtraData(); extraData[zhcfyr]._index=this._index; extraData[zhcfyr].decode(this._buffer); this._index=extraData[zhcfyr]._index; }
