...
public class Map extends Packet {
	...
	public sul.protocol.minecraft109.types.Icon[] icons = new sul.protocol.minecraft109.types.Icon[0];
	public byte colums;
	public byte rows;
	...
	public Map(int mapId, byte scale, boolean showIcons, sul.protocol.minecraft109.types.Icon[] icons, byte colums, byte rows, Tuples.ByteXZ offset, byte[] data) {
		this.mapId = mapId;
		this.scale = scale;
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(mapId) + Buffer.varuintLength(icons.length) + Buffer.varuintLength(data.length) + data.length + 7; for(sul.protocol.minecraft109.types.Icon anbm:icons){ length+=anbm.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)icons.length); for(sul.protocol.minecraft109.types.Icon anbm:icons){ this.writeBytes(anbm.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int blb5=this.readVaruint(); icons=new sul.protocol.minecraft109.types.Icon[blb5]; for(int anbm=0;anbm<icons.length;anbm++){ icons[anbm]=new sul.protocol.minecraft109.types.Icon(); icons[anbm]._index=this._index; icons[anbm].decode(this._buffer); this._index=icons[anbm]._index; }
