...
public class HubInfo extends Packet {
	 ...
	 */
	public sul.protocol.hncom1.types.GameInfo[] gamesInfo = new sul.protocol.hncom1.types.GameInfo[0];
	...
	public HubInfo(long time, long serverId, long reservedUuids, String displayName, boolean onlineMode, sul.protocol.hncom1.types.GameInfo[] gamesInfo, int online, int max, String language, String[] acceptedLanguages, String additionalJson) {
		this.time = time;
	...
	@Override
	public int length() {
		int length=Buffer.varulongLength(time) + Buffer.varulongLength(serverId) + Buffer.varulongLength(reservedUuids) + Buffer.varuintLength(displayName.getBytes(StandardCharsets.UTF_8).length) + displayName.getBytes(StandardCharsets.UTF_8).length + Buffer.varuintLength(gamesInfo.length) + Buffer.varuintLength(online) + Buffer.varintLength(max) + Buffer.varuintLength(language.getBytes(StandardCharsets.UTF_8).length) + language.getBytes(StandardCharsets.UTF_8).length + Buffer.varuintLength(acceptedLanguages.length) + Buffer.varuintLength(additionalJson.getBytes(StandardCharsets.UTF_8).length) + additionalJson.getBytes(StandardCharsets.UTF_8).length + 2; for(sul.protocol.hncom1.types.GameInfo zfznbz:gamesInfo){ length+=zfznbz.length(); };for(String ynzbzry5:acceptedLanguages){ length+=Buffer.varuintLength(ynzbzry5.getBytes(StandardCharsets.UTF_8).length)+ynzbzry5.getBytes(StandardCharsets.UTF_8).length; } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)gamesInfo.length); for(sul.protocol.hncom1.types.GameInfo zfznbz:gamesInfo){ this.writeBytes(zfznbz.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bdbvs5b=this.readVaruint(); gamesInfo=new sul.protocol.hncom1.types.GameInfo[bdbvs5b]; for(int zfznbz=0;zfznbz<gamesInfo.length;zfznbz++){ gamesInfo[zfznbz]=new sul.protocol.hncom1.types.GameInfo(); gamesInfo[zfznbz]._index=this._index; gamesInfo[zfznbz].decode(this._buffer); this._index=gamesInfo[zfznbz]._index; }
