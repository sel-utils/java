...
public class ClientboundMapItemData extends Packet {
	...
	public sul.protocol.pocket100.types.Decoration[] decorations = new sul.protocol.pocket100.types.Decoration[0];

	public ClientboundMapItemData() {}

	public ClientboundMapItemData(long mapId, int update, byte scale, Tuples.IntXZ size, Tuples.IntXZ offset, byte[] data, sul.protocol.pocket100.types.Decoration[] decorations) {
		this.mapId = mapId;
		this.update = update;
		this.scale = scale;
		this.size = size;
	...
	@Override
	public int length() {
		int length=Buffer.varlongLength(mapId) + Buffer.varuintLength(update) + Buffer.varintLength(size.x) + Buffer.varintLength(size.z) + Buffer.varintLength(offset.x) + Buffer.varintLength(offset.z) + data.length + Buffer.varuintLength(decorations.length) + 2; for(sul.protocol.pocket100.types.Decoration zvbjdlbm:decorations){ length+=zvbjdlbm.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		if(update==4){ this.writeVaruint((int)decorations.length); for(sul.protocol.pocket100.types.Decoration zvbjdlbm:decorations){ this.writeBytes(zvbjdlbm.encode()); } }
	...
	public void decode(byte[] buffer) {
		...
		if(update==4){ int bry9yrb5=this.readVaruint(); decorations=new sul.protocol.pocket100.types.Decoration[bry9yrb5]; for(int zvbjdlbm=0;zvbjdlbm<decorations.length;zvbjdlbm++){ decorations[zvbjdlbm]=new sul.protocol.pocket100.types.Decoration(); decorations[zvbjdlbm]._index=this._index; decorations[zvbjdlbm].decode(this._buffer); this._index=decorations[zvbjdlbm]._index; } }
	}
