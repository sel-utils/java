...
public class AvailableCommands extends Packet {
	...
	public int unknown1;
	public sul.protocol.pocket131.types.Command[] commands = new sul.protocol.pocket131.types.Command[0];

	public AvailableCommands() {}

	public AvailableCommands(String[] enumValues, int unknown1, sul.protocol.pocket131.types.Command[] commands) {
	...
	@Override
	public int length() {
		int length=Buffer.varuintLength(enumValues.length) + Buffer.varuintLength(unknown1) + Buffer.varuintLength(commands.length) + 1; for(String z5bzbvc:enumValues){ length+=Buffer.varuintLength(z5bzbvc.getBytes(StandardCharsets.UTF_8).length)+z5bzbvc.getBytes(StandardCharsets.UTF_8).length; };for(sul.protocol.pocket131.types.Command y9bfzm:commands){ length+=y9bfzm.length(); } return length;
	}
	...
	public byte[] encode() {
		...
		this.writeVaruint((int)commands.length); for(sul.protocol.pocket131.types.Command y9bfzm:commands){ this.writeBytes(y9bfzm.encode()); }
	...
	public void decode(byte[] buffer) {
		...
		int bnb1br=this.readVaruint(); commands=new sul.protocol.pocket131.types.Command[bnb1br]; for(int y9bfzm=0;y9bfzm<commands.length;y9bfzm++){ commands[y9bfzm]=new sul.protocol.pocket131.types.Command(); commands[y9bfzm]._index=this._index; commands[y9bfzm].decode(this._buffer); this._index=commands[y9bfzm]._index; }
	}
